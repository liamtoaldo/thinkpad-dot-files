...
battery_print() {
    ...
    if [ "$ac" -eq 1 ]; then
        ...
        if [ "$battery_percent" -gt 85 ]; then
		    ...
		    notify-send "Battery very low, charge it!" -u critical
            icon=""
        fi
# %{F#F0C674}text%{F-} is the color applied to the icon.
        echo "%{F#F0C674}$icon%{F-} $battery_percent%"
    fi
}
